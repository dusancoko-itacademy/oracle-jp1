...
async function startQuiz() {
    const xmlDoc = await loadXML('./data/java.xml');
    ...
    for (let question of questions) {
        ...
        let codeHTML = '';
        if (codeSegment) {
            codeHTML = '<pre class="code"><code class="language-java">' +codeSegment.textContent + '</code></pre>';
        }
...
function submitQuiz() {
    ...
    const finalResult = score/40 * 100 <= 0 ? 0 : score/45 * 100;
